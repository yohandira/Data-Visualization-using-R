...
brand_index_plot <-
  ...
  geom_line(
    mapping = aes(color = marketplace, size = marketplace),
    show.legend = FALSE
  ) +
  scale_color_manual(
    values = c("lazada" = "#9B5DE5", 
               "shopee" = "#FEE440",
               "tokopedia" = "#00F5D4",
               "bukalapak" = "#F15BB5",
               "blibli" = "#00BBF9")
  ...
  theme(
    ...
    plot.title = element_text(color = "#FFFFFF", size = 24, vjust = 6),
    plot.subtitle = element_text(color = "#FFFFFF", size = 16, vjust = 10),
    plot.caption = element_text(color = "#FFFFFF", size = 10, vjust = -9),
    panel.grid.major.x = element_blank(),
    panel.grid.major.y = element_line(color = "#FFFFFF", size = 0.2, linetype = "dotted"),
